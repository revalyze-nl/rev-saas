...
func (s *AnalysisServiceV2) gatherInputData(ctx context.Context, userID primitive.ObjectID) (*model.AnalysisInputV2, error) {
	...
	// Try to fetch from Pricing V2 first (new auto-imported plans)
	if s.pricingV2Repo != nil {
		v2Plans, err := s.pricingV2Repo.FindByUserID(ctx, userID)
		if err == nil && len(v2Plans) > 0 {
			for _, p := range v2Plans {
				// Use monthly equivalent for yearly plans
				price := p.PriceAmount
				if p.BillingPeriod == "yearly" && p.MonthlyEquivalentAmount > 0 {
					price = p.MonthlyEquivalentAmount
				}
				...
				input.UserPlans = append(input.UserPlans, model.PricingPlanInput{
					...
					Name:         p.PlanName,
					Price:        price,
					PriceMonthly: p.MonthlyEquivalentAmount,
					PriceYearly:  p.AnnualBilledAmount,
					Currency:     p.Currency,
	...
	// Fallback to legacy plans if no V2 plans found
	...
	// Try to fetch from Competitors V2 first (AI-discovered with pricing)
	if s.competitorV2Repo != nil {
		v2Competitors, err := s.competitorV2Repo.FindByUserID(ctx, userID)
		if err == nil && len(v2Competitors) > 0 {
			for _, c := range v2Competitors {
				...
				if c.Pricing != nil && len(c.Pricing.Plans) > 0 {
					for _, p := range c.Pricing.Plans {
	...
	// Fallback to legacy competitors if no V2 competitors found
